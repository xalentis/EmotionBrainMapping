...
def plot_comparison_charts(pos_intensities: np.ndarray, pos_assignments: np.ndarray,
                           ...
                           region_names: list):
    """
    Creates bar charts to compare region activation counts and average intensity
    between the healthy (positive) and depressed (negative) groups.
    """
    ...
    sns.barplot(data=df_combined, y='region_name', x='intensity', hue='group', ax=ax2, palette={'Healthy': 'skyblue', 'Depressed': 'salmon'})
    ax2.set_title('Comparison of Average Emotional Intensity per Brain Region', fontsize=16, fontweight='bold')
    ax2.set_xlabel('Average Emotional Intensity', fontsize=12)
...
if __name__ == "__main__":
    df = pd.read_csv('diacwoz.csv', sep="\t")
