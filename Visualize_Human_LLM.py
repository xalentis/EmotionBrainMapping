...
brain_map_diff = create_brain_map(difference_values, labels_lh, labels_rh, n_vertices_lh, n_vertices_rh)
...
stc_diff  = mne.SourceEstimate(brain_map_diff, vertices=vertices, tmin=0, tstep=1, subject=subject)
cmap = LinearSegmentedColormap.from_list('white_red', ['white', '#ff0000'])
lims_human = [0.0, human_values.max()/2, human_values.max()]
lims_llm   = [0.0, llm_values.max()/2, llm_values.max()]
lims_diff  = [0.0, difference_values.max()/2, difference_values.max()]

stc_human.plot(
    subject=subject, subjects_dir=subjects_dir, hemi='both', surface='inflated',
    time_viewer=True, colormap=cmap, clim=dict(kind='value', lims=lims_human),
    title='Human Activations'
)

stc_llm.plot(
    subject=subject, subjects_dir=subjects_dir, hemi='both', surface='inflated',
    time_viewer=True, colormap=cmap, clim=dict(kind='value', lims=lims_llm),
